...
object ErrorHandling {
  ...
  def stringOfLengthAtLeast(n: Int, fieldName: String, string: String): Either[FormError, String] =
   if(string.size >= n) Right(string)
   else Left(FormError(fieldName, s"Must be at least $n characters long"))

  def integerAtLeast(n: Int, fieldName: String, string: String): Either[FormError, Int] =
    string.toIntOption match {
	  case Some(value) =>
        if(value >= n) Right(value)
        else Left(FormError(fieldName, s"Must be at least ${n}"))
	  case None => Left(FormError(fieldName, s"Must be an integer at least ${n}"))
    }
